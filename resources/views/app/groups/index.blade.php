...
<div class="page-bar margin-bottom-20">
    ...
    <div class="page-toolbar">
        <div class="btn-group pull-right">
            <a href="{{ URL::route('app.groups.add') }}" class="btn blue margin-top-20"> 
...
<div class="portlet light bordered">
    ...
    <a href="{{ URL::route('app.groups.index', query_vars('type=0&s=0')) }}">All ({{ number_format($all) }})</a> | 
    <b>Trashed ({{ number_format($trashed) }})</b>
    @else
    <b>All ({{ number_format($all) }})</b> | 
    <a href="{{ URL::route('app.groups.index', query_vars('type=trash&s=0')) }}">Trashed ({{ number_format($trashed) }})</a>
    @endif
    ...
    <table class="table table-hover table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ str_limit($row->post_content, 100) }}</td>
                <td>
                ...
                <td>
                    ...
                    @if( Input::get('type') == 'trash')
                        <a href="#" class="popup btn btn-xs btn-primary uppercase"
                            data-href="{{ URL::route('app.groups.restore', [$row->id, query_vars()]) }}" 
                            data-toggle="modal"
                        ...
                        <a href="#" class="popup btn btn-xs btn-default uppercase"
                            data-href="{{ URL::route('app.groups.destroy', [$row->id, query_vars()]) }}" 
                            data-toggle="modal"
                            data-target=".popup-modal" 
                    ...
                    @else
                        ...
                        <a href="{{ URL::route('app.groups.edit', $row->id) }}" class="btn green btn-xs uppercase">Edit</a>
                        <a href="{{ URL::route('app.groups.permissions', $row->id) }}" class="btn blue btn-xs uppercase">Permission</a>

                        @if($row->post_status != 'actived')
                        <a href="#" class="popup btn btn-xs btn-default uppercase"
                            data-href="{{ URL::route('app.groups.delete', [$row->id, query_vars()]) }}" 
                            data-toggle="modal"
                            data-target=".popup-modal" 

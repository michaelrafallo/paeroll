...
<div class="page-bar margin-bottom-20">
    ...
    <div class="page-toolbar">
        <div class="btn-group pull-right">
            <a href="{{ URL::route('app.earnings.add', ['post_type' => $type]) }}" class="btn blue margin-top-20"> 
...
<div class="portlet light bordered">
    ...
    <a href="{{ URL::route('app.earnings.index', query_vars('type=0&s=0')) }}">All ({{ number_format($all) }})</a> | 
    <b>Trashed ({{ number_format($trashed) }})</b>
    @else
    <b>All ({{ number_format($all) }})</b> | 
    <a href="{{ URL::route('app.earnings.index', query_vars('type=trash&s=0')) }}">Trashed ({{ number_format($trashed) }})</a>
    @endif
                ...
                <td>
                    ...
                    @if( Input::get('type') == 'trash')
                        <a href="#" class="popup btn btn-xs btn-primary uppercase"
                            data-href="{{ URL::route('app.earnings.restore', [$row->id, query_vars()]) }}" 
                            data-toggle="modal"
                        ...
                        <a href="#" class="popup btn btn-xs btn-default uppercase"
                            data-href="{{ URL::route('app.earnings.destroy', [$row->id, query_vars()]) }}" 
                            data-toggle="modal"
                            data-target=".popup-modal" 
                    ...
                    @else
                        ...
                        <a href="{{ URL::route('app.earnings.edit', [$row->id, query_vars()]) }}" class="btn green btn-xs uppercase">Edit</a>

                        @if($row->post_status != 'actived')
                        <a href="#" class="popup btn btn-xs btn-default uppercase"
                            data-href="{{ URL::route('app.earnings.delete', [$row->id, query_vars()]) }}" 
                            data-toggle="modal"
                            data-target=".popup-modal" 

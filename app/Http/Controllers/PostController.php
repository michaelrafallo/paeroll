...
class PostController extends Controller
{
    ...
    public function __construct(User $user, UserMeta $usermeta, Post $post, PostMeta $postmeta, Setting $setting)
    {
        ...
        $this->single = get_types($type, 'single');
        $this->label  = get_types($type, 'label');
    ...
    public function add()
    {
        ...
        if( Input::get('_token') )
        {
            ...
            if( ! $validator->passes() ) {
                return Redirect::route('app.posts.add', ['post_type' => $type])
            ...
            if( $post->save() ) {
                return Redirect::route('app.posts.edit', [$post->id, 'post_type' => $type])
                               ->with('success',"New ".strtolower($single)." has been added!");
            } 
        }

        return view('app.posts.add', $data);
    }
    ...
    public function edit($id='')
    {
        ...
        if( Input::get('_token') )
        {
            ...
            if( ! $validator->passes() ) {
                return Redirect::route('app.posts.edit', [$id, 'post_type' => $type])
                               ->withErrors($validator)
                               ->withInput(); 
            }
            ...
            if( $post->save() ) {
                return Redirect::route('app.posts.edit', [$id, 'post_type' => $type])
                               ->with('success', strtolower($single)." has been updated!");
            } 
        }
    ...
    public function delete($id)
    {
        ...
        return Redirect::route('app.posts.index', query_vars())
